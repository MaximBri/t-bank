            ...
            renderInput={({field, errorMessage, id, inputClassName}) => {
                ...
                return (
                    <div>
                        ...
                        >
                            <Text>{selectedValues.length === 0 ? placeholder : `Выбрано участников (${selectedValues.length})`}</Text>
                            <ArrowIcon width={20} height={20} className={clsx(
...

                        {
                            isOpen ? (
                                <div
                                    className="absolute left-0 right-0 top-[calc(100%+5px)] z-20 overflow-hidden rounded-md border-[2px] border-secondary bg-secondary"
                                >
                                    ...
                                    <div className="max-h-[256px] overflow-y-auto px-[8px] py-[5px] ">
                                        <label
                                            className="flex cursor-pointer items-center gap-[14px] border-b border-primary px-[8px] py-[10px]">
                                            <input
                                                type="checkbox"
                                                checked={allSelected}
                                            ...
                                            filteredParticipants.map((participant) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        className="h-[20px] w-[20px] rounded bg-input-primary border-primary accent-yellow"
                                                    />
                                                    <Text variant="body" className="font-medium">{participant.fullName}</Text>
                                                </label>
                                            ))
                                        ) : (
